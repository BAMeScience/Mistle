...
int main() {
    ...
    cout << "target: " << target->name << " " << target->precursor_mass << endl;
    float max_dot = 0;
    ...
    for (int i = 0; i < lib->spectrum_list.size(); ++i) {
        ...
        if (abs(target->precursor_mass - candidate->precursor_mass) > 3) {
            continue;
        }
        ...
        if (candidate->peptide == "INKAIEFPIDDLKK") {
            cout << "spectrast candidate:" << candidate->peptide << " " << candidate->precursor_mass << " dot: " << scores::dot_product(search_lib->spectrum_list[0]->bins, candidate->bins) << endl;
        }
    }
    cout << "my candidate: " << best_candidate->peptide << " " << best_candidate->precursor_mass << " dot: " << max_dot << endl;
